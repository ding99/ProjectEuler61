...
bool Cubes::Search(unsigned long long n, unsigned int range) {
	...
	if(valid(cube))
		cout << "DSP_1(" << n << "," <<  cube << ") s(" << s << ") ";

	if (s.length() > digits) {
		int num = candidates.size();

		cout << "<digits_1 " << digits << "-" << cubes.size() << " candidates " << num << "> ";
	...
	if(sets.size() + 2 >= range)
		cout << "[lower " << sets.size() + 1 << "," << n << "," << cube << "] ";
...
void Cubes::Start() {
	...
	bool found = false;
	for (unsigned long long n = 1; n < 60000; n++) {
		if ((n % 100) == 0) cout << n << " ";
